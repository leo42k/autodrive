...
library(animation)
saveGIF(for (i in seq(1000, 2000, 20)) plot(as.cimg(aperm(image[image_names][i,,,], c(4,3,1,2)))), "test.gif", interval = 1)
?saveGIF

install.packages("PET")
library(PET)

targetimage <- (aperm(image[image_names][10001,,,], c(4,3,1,2)))
plot(targetimage %>% as.cimg)
dim(targetimage)
hough(targetimage)
