...
names(women)
nrow(women)
women <- cbind(women, height1, height2)
women <- women[,1:2]
summary(fm1 <- lm(weight ~ ns(height[1:9], df = 5) + ns(height[10:15], df = 5), data = women))


women
plot(women[,1:2], xlab = "Height (in)", ylab = "Weight (lb)")
ht <- seq(57, 73, length.out = 200)
lines(ht, predict(fm1, data.frame(height= ht, norm = 1)))
lines(ht, predict(fm1, data.frame(height= 0, norm = seq(-1, 1, length.out = 200))))

lines(ht, predict(fm1, data.frame(height1= ht, height= 0, height2= 1)))
lines(ht, predict(fm1, data.frame(height1= ht, height= 0, height2= 1)))
